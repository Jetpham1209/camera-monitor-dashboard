...
if [ -f "$legacy_pid_file" ]; then
  ...
  if [ -n "$legacy_pid" ] && kill -0 "$legacy_pid" >/dev/null 2>&1; then
    ...
    echo "Stopped legacy Frame Capture PID ${legacy_pid}."
  fi
...
if docker ps -a --format '{{.Names}}' | grep -Fx "$CONTAINER_NAME" >/dev/null 2>&1; then
  ...
  echo "Stopped Frame Capture service container ${CONTAINER_NAME}."
  exit 0
fi

echo "Frame Capture service container is not running: ${CONTAINER_NAME}"
